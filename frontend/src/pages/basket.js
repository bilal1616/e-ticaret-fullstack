...
function BasketComponent(){
    ...
    const getAll = async () => {
        ...
        setTotal(totalC.toFixed(2));
    }


    const remove = async (_id) => {
        let confirm = window.confirm("Sepetteki Ürünü Silmek İstiyormusunuz ?");
        if(confirm){
            let model  = {_id: _id};
                                    ...
                                    <tbody>
                                        {baskets.map((basket,index)=>(
                                            <tr key={index}>
                                                ...
                                                <td>
                                                    <img src={"http://localhost:5000/" + basket.products[0].imageUrl} width="75px"/>
                                                </td>
                                                <td>1</td>
                                                <td>{basket.products[0].price}</td>
                                                <td>
                                                    <button onClick={()=>remove(basket._id)} className='btn btn-outline-danger'>Sil</button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
